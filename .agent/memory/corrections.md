...
- **Syntax Integrity (CORRECTION — 2026-02-20)**: ❌ The AI introduced a `Parsing ecmascript source code failed` error in `BookingViewModal.tsx` by accidentally doubling the closing `);` bracket during a file replacement. This broke the build. ✅ NEW RULE ENFORCED: After any non-trivial code edit, the AI MUST immediately run `npm run lint` or `npm run build` (or at least check the file structure) to verify syntax integrity before declaring a task "Complete". Careless syntax errors are a violation of the "Senior Dev" persona.
- **Verification Failure (CRITICAL FAILURE — 2026-02-20)**: ❌ The AI claimed the Admin Modal redesign was "Verified" and "Complete" while the build was actually broken due to a syntax error. It skipped the mandatory Rule 4 (npm run build). ✅ SYSTEMIC FIX: A "Hard Verification Wall" has been added to `.clinerules`. Handover is now impossible without sharing terminal build logs.

- **Move Verification Failure (2026-02-20)**: ? AI assumed move command succeeded without checking for 'ObjectNotFound' errors. ? NEW RULE: Always verify folder existence using Test-Path or ls before and after operations to prevent 'Directory Shadowing'.

- [2026-02-20] Incomplete Theme Overrides: When a user requests a pure color state (like all white), do not rely on global theme variables. Use aggressive tailwind overrides (! or g-white!) on all sub-components to prevent theme leakage from parent layouts.
